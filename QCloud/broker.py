...
class ParallelBroker(BaseBroker):
    ...
    def run(self):
        ...
        """
        if self.job.num_qubits > max([d.number_of_qubits for d in self.devices]):
            # Allocate large job across multiple devices
            if self.qcloud.printlog: 
                print(f"{self.env.now:.2f}: Job #{self.job.job_id} requires {self.job.num_qubits} qubits. Allocating across devices.")
            yield from self.env.qcloud.allocate_job(self.job, self.devices)
        else:
