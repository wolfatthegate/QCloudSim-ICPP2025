...
class IBM_QuantumDevice(QuantumDevice):
    ...
    def __init__(self, name, nodes_file_name, pos_file_name, env, maintenance_interval, maintenance_duration, maintenance_switch, clops, qvol, median_T1, median_T2, processor_type, cali_filepath=None, printlog=True):
        ...
        self.calculate_avg_error()
        self.error_score = self.calculate_error_score()    

    def calculate_avg_error(self):
        try:
            self.avg_single_qubit_error = self.single_qubit_gate_errors.get("sx", None)
            self.avg_two_qubit_error = sum(self.two_qubit_gate_errors.values()) / len(self.two_qubit_gate_errors)

            if self.readout_errors and len(self.readout_errors) > 0:
                self.avg_readout_error = sum(self.readout_errors) / len(self.readout_errors)
            else:
                self.avg_readout_error = None

        except KeyError as e:
            if self.printlog:
                print(f"KeyError encountered: {e}")
            self.avg_single_qubit_error = None
            self.avg_readout_error = None
    ...
    def estimate_fidelity(self, job):
        ...
        two_qubit_fidelity = (1 - self.avg_two_qubit_error) ** two_qubits
        ...
        # Combined fidelity
        estimated_fidelity = single_qubit_fidelity * readout_fidelity * two_qubit_fidelity
    ...
    def calculate_error_score(self): 
        return (0.5 * self.avg_readout_error) + (0.3 * self.avg_single_qubit_error) + \
                        (0.2 * self.avg_two_qubit_error)
